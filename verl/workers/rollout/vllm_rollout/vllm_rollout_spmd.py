...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        kwargs = dict(
            n=1,
            logprobs=20,  # can be set to 0 and let actor to recompute
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
                ...
                for sample_id in range(len(output.outputs)):
                    ...
                    if self.config.calculate_log_probs:
                        ...
                        for i, logprob in enumerate(output.outputs[sample_id].logprobs):
                            logprob_list =[lp.logprob for lp in list(logprob.values())]
                            token_entropy = -sum(p * np.exp(p) for p in logprob_list if p > -100)
                            token_entropy_ls.append(token_entropy)
                            curr_log_prob.append(logprob[response_ids[i]].logprob)
                        rollout_log_probs.append(curr_log_prob)
                        token_entropys.append(token_entropy_ls)
            ...
            if self.config.calculate_log_probs:
                ...
                token_entropys = pad_2d_list_to_length(
                    token_entropys, 0.0, max_length=self.config.response_length
                ).to(idx.device)
                token_entropys = token_entropys.to(torch.float32)
        ...
        if self.config.calculate_log_probs:
            ...
            batch["token_entropys"] = token_entropys
